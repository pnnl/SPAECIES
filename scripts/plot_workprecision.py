...
# SINGLE_PLOT = flag for plotting all simulations on one plot
##########################################################################################
    ...
    for order in orders:
        if MRI_FLAG[idx] == True:
            ...
            for dt_slow in slow_timesteps:
                # gather files associated with order and dt_slow
...
# markers and linestyles to cycle through
markers = itertools.cycle(('o', 'v', 's', 'd')) 
linestyles = itertools.cycle(('-', '--', '-.', '..')) 

# colors to cycle through
colors = {'1': 'tab:purple', '2': 'tab:blue', '3': 'tab:orange', '4': 'tab:green', '5': 'tab:gray'}

if SINGLE_PLOT:
    fig, ax = plt.subplots(1, 2, figsize=(20,8))
    savename = SAVE_PATH + "/workprecision_all.png"
...
for idx, SIM_NAME in enumerate(SIM_NAMES):
    ...
    linestyle_i = next(linestyles)
    
    if not SINGLE_PLOT:
        fig, ax = plt.subplots(1, 2, figsize=(20,8))
        savename = SAVE_PATH + "/workprecision_" + SIM_NAME + ".png"
    
    if MRI_FLAG[idx] == True:
        for order in errors_sim_i:
            for dt_slow in errors_sim_i[order]:
                marker_i = next(markers)
                ...
                ax[0].loglog(timesteps_sim_i[order][dt_slow], errors_sim_i[order][dt_slow], linestyle=linestyle_i, marker=marker_i, label=LABELS[idx] + order + ", $\Delta t_{slow}=$" + dt_slow_print, color=colors[order])
                ax[1].loglog(walltimes_sim_i[order][dt_slow], errors_sim_i[order][dt_slow], linestyle=linestyle_i, marker=marker_i, label=LABELS[idx] + order + ", $\Delta t_{slow}=$" + dt_slow_print, color=colors[order])
    else:
        for order in errors_sim_i:
            ax[0].loglog(timesteps_sim_i[order], errors_sim_i[order], marker='s', linestyle=linestyle_i, label=LABELS[idx] + order, color=colors[order])
            ax[1].loglog(walltimes_sim_i[order], errors_sim_i[order], marker='s', linestyle=linestyle_i, label=LABELS[idx] + order, color=colors[order])
