            ...
            do
                ...
                do 
                    ...
                    then
                        ...
                        SIMULATION_NAME="${INTEGRATION_TYPES[kk]}_fixedstep_C3regularizedevapandlambdar_nolimiter"
        ...
        do
            ...
            do
                ...
                do
                    for j in $(seq 0 $((${#EPSILON_QSAT_FACS[@]} - 1)))
                    do 
                        for jj in $(seq 0 $((${#EPSILON_SELF_COLLS[@]} - 1)))
                        do 
                            if [[ $REGULARIZE_LAMBDAR == "true" && $REGULARIZE_QSAT == "true" ]]
                            then
                                SIMULATION_NAME="${INTEGRATION_TYPES[kk]}_fixedstep_C3regularizedevapandlambdar"
                                OUTPUT_FILE="${SAVE_DIR}/rainshaft_${SIMULATION_NAME}_finaltime${FINAL_TIME}_qsmall${QSMALLS[ii]}_epsilonqsat${EPSILON_QSAT_FACS[j]}_epsilonselfcoll${EPSILON_SELF_COLLS[jj]}_order${ORDERS[k]}_dt${TIMESTEPS[i]}.nc"
                            else 
