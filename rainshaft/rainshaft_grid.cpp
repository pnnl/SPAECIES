...
const std::size_t num_e3sm_levs_init = 72;
const double e3sm_ilev_init[num_e3sm_levs_init+1] = {
  0.100000003197544, 0.147650822913689, 0.218007648100008,
  0.321890076141867, 0.475273331103897, 0.701744962025609,
  1.03613217805539, 1.52985763845446, 2.25884732035832, 3.3352065502282,
  4.92445975982147, 7.01243897441468, 9.74236978346886, 13.2052046776003,
...
// const std::size_t num_e3sm_levs = (72+1)*2 - 1;
const std::size_t num_e3sm_levs = 300;
double e3sm_ilev[num_e3sm_levs];
...
void generate_pcoord(const double* e3sm_ilev_init, double* e3sm_ilev) {
  ...
  //   if (i % 2 == 0) {
  //     e3sm_ilev[i] = e3sm_ilev_init[int(i/2)];
  //   }
  //   else {
  //     e3sm_ilev[i] = 0.5*(e3sm_ilev_init[int((i-1)/2)] + e3sm_ilev_init[int((i-1)/2)+1]);
  //   }
  // }
...
RainshaftGrid make_e3sm_like_grid(RainshaftConstants constants,
  ...
  generate_pcoord(e3sm_ilev_init, e3sm_ilev);
