  ...
  for (const std::size_t &icase : cases_to_run) {
    ...
    const auto intg = [&]() -> std::unique_ptr<RainshaftIntegrator> {
      if (method_type == "explicit") {
        return std::make_unique<ExplicitIntegrator>(constants, grid, &all_processes, state_descs, tend_descs, dt, order, rel_tol, postprocess, regularize_lambdar, steps_per_output);
      } else if (method_type == "implicit") {
        return std::make_unique<IMEXIntegrator>(constants, grid, &all_processes, nullptr, state_descs, tend_descs, dt, order, rel_tol, postprocess, regularize_lambdar, steps_per_output);  
      } else if (method_type == "imex") {
        return std::make_unique<IMEXIntegrator>(constants, grid, &partition_1_processes, &partition_2_processes, state_descs, tend_descs, dt, order, rel_tol, postprocess, regularize_lambdar, steps_per_output);
      } else if (method_type == "mri") {
        return std::make_unique<MRIIntegrator>(constants, grid, &partition_1_processes, &partition_2_processes, nullptr, state_descs, tend_descs, dt_partition_1, dt, order, rel_tol, postprocess, regularize_lambdar, steps_per_output);
      } else if (method_type == "splitting") {
        return std::make_unique<OperatorSplittingIntegrator>(constants, grid, &partition_1_processes, &partition_2_processes, state_descs, tend_descs, dt, dt_partition_1, dt_partition_2, cfl_substep, order, rel_tol, postprocess, regularize_lambdar, steps_per_output);
      } else if (method_type == "forcing") {
        return std::make_unique<ForcingIntegrator>(constants, grid, &partition_1_processes, &partition_2_processes, state_descs, tend_descs, dt, dt_partition_1, dt_partition_2, cfl_substep, postprocess, regularize_lambdar, steps_per_output);
        ...
        std::shared_ptr<ExplicitIntegrator> local_intg = std::make_shared<ExplicitIntegrator>(constants, grid, &partition_2_processes, state_descs, tend_descs, dt, 1, rel_tol, false, regularize_lambdar);
