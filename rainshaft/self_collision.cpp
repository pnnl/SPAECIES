                                  ...
                                  SUNMatrix jac) const
{
  ...
  for (std::size_t il = 0; il != grid.nlev; ++il)
  {
    ...
    elem(i_nr, i_t) += 5.78 * b * state.nr[il] * state.qr[il] * grid.p_mid[il] /
                      (pow(state.t[il], 2) * constants.rdry * (1.0 + state.q[il] / constants.epsilon_h2o));

    elem(i_nr, i_q) += 5.78 * b * state.nr[il] * state.qr[il] * grid.p_mid[il] /
    ...
    elem(i_nr, i_nr) += -5.78 * state.qr[il] * dvars.rho_dry[il] * (db * state.nr[il] + b);

    elem(i_nr, i_qr) += -5.78 * state.nr[il] * dvars.rho_dry[il] * (-db * state.qr[il] + b);
  }
}
