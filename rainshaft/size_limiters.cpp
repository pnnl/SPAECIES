...
double diameter_to_nr_fac(const RainshaftConstants& constants, double diameter, double mu) {
  ...
  // shape parameter mu.
  double lambdar = (mu + 1.) / diameter;
  double mu_polynomial = (mu + 1.) * (mu + 2.) * (mu + 3.);
...
SizeLimiters::SizeLimiters(const RainshaftConstants& constants,
  ...
  : min_nr_fac(diameter_to_nr_fac(constants, max_diameter, mu)),
    max_nr_fac(diameter_to_nr_fac(constants, min_diameter, mu))
{
}
